...
def get_prompted_assistant_without_task(role, proficiency_level, response_style, response_template, response_length, code_correction_style):
    ...
    system_prompt = (
        f"You are taking the role of a {role} for a {proficiency_level} Python software developer who seems to seek for some help. If they need help with something else than Python, assist them with any other programming language and any task related to software development or software engineering, too."
        f"{text_response_style}"
        f"{text_response_length}"
        f"If the developer asks you to provide code, do so. If you would like to include code in your answers, without the developer explicitly asking for code before, ask the developer first, if you should provide it."
        f"{text_response_template}"
